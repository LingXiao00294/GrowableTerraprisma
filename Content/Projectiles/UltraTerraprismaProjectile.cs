...
{
    public class UltraTerraprismaProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            player.AddBuff(ModContent.BuffType<UltraTerraprismaBuff>(), 3600);

            if (player.dead)
            {
                ultra.ultraMinionActive = false;
            }

            if (ultra.ultraMinionActive)
            {
                Projectile.timeLeft = 2;
            }

            // --- 继承 gtprisma 被动能力 ---
            if (ultra.defeatedBossTypes.Contains(NPCID.WallofFlesh))
            {
                Lighting.AddLight(Projectile.Center, 1.2f, 0.9f, 1.8f);
            }

            if (ultra.defeatedBossTypes.Contains(NPCID.QueenBee))
            {
        ...
        /// <summary>
        /// 行为叠加层 — 遍历已解锁的行为并执行 AI。
        /// Phase 6: 空方法体。Phase 7: 接入 UprismaBehaviorRegistry。
        /// </summary>
        private void RunBehaviors(Player player)
        {
            // Phase 7 将在此调用 UprismaBehaviorRegistry.GetUnlocked(player)
            // foreach (var behavior in unlockedBehaviors)
            //     if (behavior.CanRun(Projectile))
            //         behavior.AI(Projectile);
        }

        #region AI_156 State Machine (移植自 gtprisma)

        private void AI_156_Think(Player player, List<int> blacklist)
        {
            var ultra = player.GetModPlayer<GrowableTerraprismaPlayer>();
            bool fetchUnlocked = ultra.defeatedBossTypes.Contains(NPCID.TheDestroyer);
        ...
        #region Rendering (EmpressBladeDrawer 顶点条带 + 精灵绘制)
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            // EmpressBladeDrawer 切到 Immediate 模式，恢复 Deferred
        ...
        private void DrawTrail(Player player)
        {
            ...
            drawer.ColorStart = Projectile.GetFairyQueenWeaponsColor(0f, 0f, hueBase % 1f);
            drawer.ColorEnd = Projectile.GetFairyQueenWeaponsColor(0f, 0f, (hueBase + 0.5f) % 1f);
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            // 继承 gtprisma: 双子魔眼 — 微型棱镜
            if (ultra.defeatedBossTypes.Contains(NPCID.Retinazer)
                && ultra.defeatedBossTypes.Contains(NPCID.Spazmatism))
            {
                ultra.miniPrismHitCounter++;
                if (ultra.miniPrismHitCounter >= 20)
                {
                    ultra.miniPrismHitCounter = 0;
        ...
        {
            var ultra = Main.player[Projectile.owner].GetModPlayer<GrowableTerraprismaPlayer>();
            // 继承 gtprisma: 机械骷髅王 — 穿甲 +10
            if (ultra.defeatedBossTypes.Contains(NPCID.SkeletronPrime))
            {
